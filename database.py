...
def crear_tablas():
    with conectar() as conn:
        with conn.cursor() as cursor:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS avance_curricular (
                    id SERIAL PRIMARY KEY,
                    estudiante_id INTEGER NOT NULL REFERENCES estudiantes(id) ON DELETE CASCADE,
            ...
            migraciones = [
                ...
                *[f"ALTER TABLE diagnosticos ADD COLUMN IF NOT EXISTS pregunta_{i} INTEGER" for i in range(1, 21)],
                ...
                "ALTER TABLE avance_curricular ADD COLUMN IF NOT EXISTS creditos INTEGER",
                "ALTER TABLE avance_curricular ADD COLUMN IF NOT EXISTS veces INTEGER DEFAULT 1",
                "ALTER TABLE avance_curricular ADD COLUMN IF NOT EXISTS nota REAL",
                "ALTER TABLE avance_curricular ADD COLUMN IF NOT EXISTS estado TEXT",
                "ALTER TABLE avance_curricular ADD COLUMN IF NOT EXISTS origen TEXT",
            ]
...
def limpiar_avance_curricular(estudiante_id: int):
    _execute("DELETE FROM avance_curricular WHERE estudiante_id = %s", (estudiante_id,))
    return True


def guardar_avance_curricular(
    estudiante_id: int,
    curso_id: Optional[int],
    ciclo: str,
    codigo_curso: str,
    ...
    _execute(
        """
        INSERT INTO avance_curricular (
            estudiante_id, curso_id, ciclo, codigo_curso, nombre_curso,
            creditos, veces, nota, estado, origen, fecha_registro
        )
...
def listar_avance_curricular(estudiante_id: int):
    return _fetchall(
        """
        SELECT id, ciclo, codigo_curso, nombre_curso, creditos, veces, nota, estado, origen, fecha_registro
        FROM avance_curricular
        WHERE estudiante_id = %s
        ORDER BY ciclo DESC NULLS LAST, codigo_curso ASC, id DESC
        """,
        (estudiante_id,),
    )


def obtener_indicador_riesgo_historico(estudiante_id: int):
    """Resume antecedentes académicos importados desde Avance curricular.

    Regla central solicitada: si un alumno llevó cursos 3 o más veces, ya tuvo riesgo antes;
    por tanto, este historial sube el riesgo del diagnóstico actual.
    """
    fila = _fetchone(
        """
        SELECT
            COUNT(*) AS total_registros,
            COALESCE(MAX(veces), 0) AS max_veces,
            COALESCE(SUM(CASE WHEN veces >= 3 THEN 1 ELSE 0 END), 0) AS cursos_3_mas,
            COALESCE(SUM(CASE WHEN veces > 3 THEN 1 ELSE 0 END), 0) AS cursos_mas_3,
            COALESCE(SUM(CASE WHEN nota IS NOT NULL AND nota < 11 THEN 1 ELSE 0 END), 0) AS notas_desaprobadas,
            COALESCE(AVG(CASE WHEN nota IS NOT NULL THEN nota END), 0) AS promedio_historico
        FROM avance_curricular
        WHERE estudiante_id = %s
        """,
        (estudiante_id,),
    ...
    if not fila:
        return {
            ...
            "ajuste_riesgo": 0,
            "resumen": "Sin avance curricular importado.",
        }
    total, max_veces, cursos_3_mas, cursos_mas_3, notas_desaprobadas, promedio_hist = fila
    total = int(total or 0)
    max_veces = int(max_veces or 0)
    cursos_3_mas = int(cursos_3_mas or 0)
    cursos_mas_3 = int(cursos_mas_3 or 0)
    notas_desaprobadas = int(notas_desaprobadas or 0)
    promedio_hist = float(promedio_hist or 0)

    ajuste = 0
    if cursos_mas_3 > 0:
        ajuste += min(25, 15 + (cursos_mas_3 - 1) * 5)
    elif cursos_3_mas > 0:
        ajuste += min(18, 10 + (cursos_3_mas - 1) * 4)
    if max_veces >= 4:
        ajuste += 5
    ajuste += min(15, notas_desaprobadas * 2)
    if 0 < promedio_hist < 11:
        ajuste += 5
    ajuste = min(35, ajuste)

    resumen = (
        f"Avance curricular: {total} registros; máximo de veces en un curso: {max_veces}; "
        f"cursos llevados 3 o más veces: {cursos_3_mas}; desaprobaciones detectadas: {notas_desaprobadas}."
    )
    return {
        "total_registros": total,
        "max_veces": max_veces,
        "cursos_3_mas": cursos_3_mas,
        "cursos_mas_3": cursos_mas_3,
        "notas_desaprobadas": notas_desaprobadas,
        "promedio_historico": round(promedio_hist, 2) if promedio_hist else 0,
        "ajuste_riesgo": ajuste,
        "resumen": resumen,
    }
