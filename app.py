...
if menu == "Inicio":
    ...
    if estudiante_id is not None:
        ...
        if avance_actual:
            st.subheader("📈 Avance curricular importado")
            df_avance = pd.DataFrame(avance_actual, columns=["ID", "Ciclo", "Código", "Curso", "Créditos", "Veces", "Nota", "Estado", "Origen", "Fecha"])
            st.dataframe(df_avance, use_container_width=True)
            indicador = obtener_indicador_riesgo_historico(estudiante_id)
            if indicador.get("ajuste_riesgo", 0) > 0:
                st.warning(f"Este historial se usará como indicador adicional de riesgo: {indicador.get('resumen')}")
    ...
    if estudiante_id is not None:
        ...
        historial_riesgo = obtener_indicador_riesgo_historico(estudiante_id)
        if historial_riesgo.get("total_registros", 0):
            ajuste = historial_riesgo.get("ajuste_riesgo", 0)
            if ajuste > 0:
                st.warning(f"Indicador histórico activo: {historial_riesgo.get('resumen')} Ajuste sugerido al riesgo: +{ajuste} puntos.")
            else:
        ...
        with st.form("form_diagnostico_ia"):
            ...
            if st.form_submit_button("🤖 Generar diagnóstico con IA y guardar"):
                ...
                with st.spinner("AURA está generando el diagnóstico con IA..."):
                    resultado = generar_diagnostico_ia(nombre_estudiante, st.session_state[f"{pref}horas"], st.session_state[f"{pref}promedio"], respuestas, historial_riesgo)
